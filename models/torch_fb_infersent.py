...
class InferSentInterface(object):

    def __init__(self):
        if os.path.exists('./data_utils/infersent1.pkl'):
            modelpath = './data_utils/infersent1.pkl'
        else:
            modelpath = '../input/infersent1.pkl'
        ...
        self.infersent.load_state_dict(torch.load(modelpath))

        if os.path.exists('./data_utils/crawl-300d-2M.vec'):
            W2V_PATH = './data_utils/crawl-300d-2M.vec'
        else:
            W2V_PATH = '../input/crawl-300d-2M.vec'
    ...
    def encode(self, text, flatten=False):
        if isinstance(text, list):
            ...
            for sentence in text:
                vec = self.infersent.encode([sentence])[0]
                sentences_vectors.append(vec)

            return sentences_vectors
        else:
            return self.infersent.encode([text])[0]

    def load_vocab(self, sentences):
        self.infersent.build_vocab(sentences, tokenize=True)

    def update_vocab(self, sentences):
        self.infersent.update_vocab(sentences, tokenize=True)
